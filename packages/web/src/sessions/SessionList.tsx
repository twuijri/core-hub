...
import { IconArchive, IconGrip, IconPin, IconTrash } from '../ui/icons.js';
...
export function SessionList({ onOpen }: { onOpen?: () => void }) {
  ...
  const onDelete = async (session: Session) => {
    if (!window.confirm(t('sessions.confirm_delete', { title: sessionTitle(session, t) }))) return;
  ...
  return (
    <div className="flex flex-col gap-2" data-testid="session-list">
      ...
      {sessions.data && items.length === 0 && (
        <Notice>{filter ? t('sessions.none_match') : t('sessions.empty')}</Notice>
      )}
          ...
          <ul className="flex flex-col gap-0.5" aria-label={t('nav.chat')}>
            {items.map((session) => (
              <SessionRow
                ...
                onArchive={() => update.mutate({ id: session.id, patch: { archived: true } })}
