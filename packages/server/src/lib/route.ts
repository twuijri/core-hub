...
export function defineRoute(
  ...
  app.route({
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // then send an error after the headers, which throws out of the process (DECISIONS §92).
 ...
 * the headers, which throws out of the process (DECISIONS §92). A client that went away is not an
