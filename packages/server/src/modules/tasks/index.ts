 ...
 * profile it happens to be in (DECISIONS §30). Without one (tests that compose no names),
  ...
  registerRoutes(app: FastifyInstance) {
    ...
    defineRoute(app, deps, {
      ...
      handler: async (request, { query }) => {
        ...
        // `profiles=all` is what the board is anyway (DECISIONS §30); with `profile` it says
    ...
    defineRoute(app, deps, {
      ...
      handler: (request, { query }) => {
        ...
        // `profiles=all` (ADR 0016, DECISIONS §30): every workspace this person may enter —
