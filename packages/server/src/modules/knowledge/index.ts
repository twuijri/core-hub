...
export interface AttachmentsPort {
  ...
  /**
   * A one-hour address a media element plays one file of a folder from (decision §92): the
...
/** The file stream tickets of one hub (decision §92); the attachment ones are §90's. */
...
export const knowledgeModule = defineModule({
  ...
  async registerRoutes(app: FastifyInstance) {
    ...
    // `knowledge.createWorkspaceFileStream` made (§92): opened again, by the working-file rules,
    // on every read.
    defineRoute(app, deps, {
