...
import type { AgentEvent, AgentSession, CompressOutcome, PromptInput } from './types.js';
...
export class HermesTuiSession implements AgentSession {
   ...
   * `/skill <name>`) is Hermes's to carry out, not text for the model (decision §50). Hermes's
  ...
  /**
   * Hermes's `session.compress` (decision §50): only between turns. Its answer carries the
...
/** The commands Hermes carries out itself, by the word after `/` (decision §50). */
