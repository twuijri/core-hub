/**
 * Incoming webhooks against **the real Hermes** from the image (contract decision §91).
 *
...
describe.skipIf(!image)('Hermes incoming webhooks (real Hermes gateway; set COREHUB_HERMES_IMAGE)', () => {
  const asked: string[] = [];
  ...
  it('a signed POST to the hub reaches Hermes, which runs the agent with the route’s prompt', async () => {
    ...
    while (Date.now() < until && !asked.some((m) => m.includes('Deploy of core-hub finished: green'))) {
      await new Promise((resolve) => setTimeout(resolve, 500));
    }
    expect(
    ...
    expect(tested.json()).toMatchObject({ status: 202, body: { status: 'accepted', event: 'test' } });
  }, 200_000);
});
