...
/**
 * What a request waits for when the asker does not say (§86): a computer's helper reads a file
...
/** What an agent's run may ask the person's own computer for (§86). */
 ...
 * "offline" at once instead of a wait for a device that cannot hear it (§86). A phone's
...
export function checkResult(capability: CapabilityKind, result: unknown): Record<string, unknown> {
  ...
  if (capability === 'apps') {
    // A program's call is either finished (its MCP result) or still running (§86).
...
export class DeviceRequestService {
  ...
  create(input: {
    ...
    // Nobody would hear it: said at once, as a failure the asker can tell the person (§86).
