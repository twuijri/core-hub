// `pnpm contract:test`: speech through the generated TypeScript client (contract decision §63) —
...
// schema the contract gives that status. Before §63 `models.transcribe` answered `501`.
...
describe.skipIf(!doc)('contract: speech', () => {
  ...
  it('voices: the documented list, labelled, narrowed by model; a preview names its model (§94)', async () => {
    const { data: speech } = await call('models.getSpeech', 200);
    const tts = (
  ...
  it('presets: an address example and the key a family already holds (§94)', async () => {
