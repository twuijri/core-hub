...
## 78. A device says what it is and what stops its push; a name a person gives it stays
...
- Apps send them when pairing or registering and again at each launch with `devices.update` —
  **never the name** there. A name set with `devices.update` is the person's (`renamed_at`,
  migration `0025`): pairing or registering the same device again keeps it; unlinking forgets it.
