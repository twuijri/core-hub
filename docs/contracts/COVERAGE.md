...
| Background button (top bar, beside the pending-actions bar; DECISIONS §49): the person's running chat, task, schedule and workflow runs, jobs and subagents in every profile, and what finished in the last 24 hours | `background.list` (`profiles=all`) | `background.stop` (in the item's profile), open the item where it lives | `session.updated`, `run.*`, `subagent.*`, `schedule_run.*`, `workflow_run.*`, `job.*` (each only makes the client read the list again) |
