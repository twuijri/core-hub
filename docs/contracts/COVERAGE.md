...
| 10 | `settings` › **Proxy** / **Compression** / **Privacy** | `auth.getProfileSettings`; since §56 the proxy and «hide ids from the model» are Hermes's own: `agents.getSettings` sections `network` and `privacy` (the hub's `proxy` and `privacy` are deprecated) | `auth.updateProfileSettings` (one section per save); Privacy's switch → `agents.updateSettings` section `privacy` | `job.*` |
...
| 33 | `hermesSettings` — Settings (Hermes's own keys per profile: Agent runtime / Memory / Approvals / Network proxy / Privacy from `sections`, each field with `help` and Hermes's `default`; §56) and «Waiting for review» | `agents.getSettings`, `agents.listPendingWrites` | `agents.updateSettings` (one section per save; `restart_job_id` for the default profile's proxy), `agents.approvePendingWrite`, `agents.rejectPendingWrite` | `agent.updated`, `job.*` |
