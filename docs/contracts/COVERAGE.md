...
| Pending-actions bar (approvals, questions and workflow gates anywhere; senders waiting to pair, for admins) | `sessions.listApprovals` (once per profile the person may enter), `agents.listPairing` and `agents.listPendingWrites` (admins, the profile they are in; DECISIONS §102) | `sessions.respondApproval`, `agents.approvePendingWrite`, `agents.rejectPendingWrite`, open the item where it lives (a room's in its room), open Global Agent | `approval.requested`, `approval.resolved` |
| Notices (bell) and push | `notify.listNotices`, `notify.getPreferences`, `devices.getPushConfig` | `notify.updateNotice`, `notify.markAllRead`, `notify.setPreferences`, `notify.sendTestNotice`, `devices.register` (a browser), `devices.registerPush` / `devices.unregisterPush` | `notice.created`, `notice.updated` |
...
| Background button (top bar, beside the pending-actions bar; DECISIONS §56): the person's running chat, task, schedule and workflow runs, jobs and subagents in every profile, and what finished in the last 24 hours | `background.list` (`profiles=all`) | `background.stop` (in the item's profile), open the item where it lives | `session.updated`, `run.*`, `subagent.*`, `schedule_run.*`, `workflow_run.*`, `job.*` (each only makes the client read the list again) |
...
| 3 | `deviceConnections` — Device connections › **App › Direct** | `devices.list`, `devices.get` | `auth.createPairing` (show QR), `auth.getPairing`, `auth.cancelPairing`, `devices.update` (rename), `devices.unlink`, `devices.testPush` | `pairing.claimed`, `device.linked`, `device.updated`, `device.unlinked`, `device.online`, `device.offline` |
| 3 | `deviceConnections` › **App › Message push** | `devices.get` (`push`) | `devices.registerPush`, `devices.unregisterPush` | `device.updated` |
| 3 | `deviceConnections` › **Devices** › Push senders (admin) | `devices.listPushSenders` | `devices.setPushSender`, `devices.deletePushSender` | — |
| 3 | `linkedHubs` — Settings › Linked hubs (admin, web and desktop; ADR 0026) | `devices.listPeers`, `devices.listPeerShares`, `devices.listPeerAgents`, `devices.listPeerEvents` | `devices.createPeerInvite`, `devices.requestPeer`, `devices.updatePeer` (approve, rename, on/off, questions per hour), `devices.deletePeer` (unlink or refuse), `devices.setPeerShare`, `devices.askPeerAgent` | — |
...
| 10 | `settings` › **Proxy** / **Compression** / **Privacy** | `auth.getProfileSettings`; since §58 the proxy and «hide ids from the model» are Hermes's own: `agents.getSettings` sections `network` and `privacy` (the hub's `proxy` and `privacy` are deprecated) | `auth.updateProfileSettings` (one section per save); Privacy's switch → `agents.updateSettings` section `privacy` | `job.*` |
| 10 | `settings` › **Models** (provider keys only) | `models.listProviders` | `models.updateProvider` (`api_key`) | — |
| 10 | `settings` › **This device** | `devices.get` (`this_device`), `auth.getPreferences` (`voice`), `updates.check`, `notify.getPreferences`, `models.getSpeech` (the voice test), `devices.getRelay` (desktop, local mode: Reach from outside) | `devices.update` (name, capabilities, app version), `auth.setPreferences` (voice modes, dictation language, reasoning effort), `updates.download` (Range), `devices.registerPush`, `models.transcribe` / `models.synthesize` (the voice test), `devices.setRelay` | `device.updated`, `notice.created` (`update_available`) |
...
| 12 | `usage` — Usage | `audit.getUsage` (`days`, `profiles=all`, `agent_id`, `utc_offset_minutes`; decision §50); `preferences.show_cost` for the cost toggle | — | — |
| 13 | `performance` — Performance (admin) | `audit.getLivePerformance` — polled every 5 s while visible (DECISIONS §51) | — | — |
| 14 | `skillsUsage` — Skills Usage | `audit.getSkillUsage` (`days`, `profiles=all`, `agent_id`, `utc_offset_minutes`; decision §50); `getReport` `kind=skills` answers the same body | — | — |
...
| 16b | `files` — Files (owner and admin; the selected profile's working folder, DECISIONS §65) | `knowledge.listWorkspaceFiles`, `knowledge.readWorkspaceText`, `knowledge.downloadWorkspaceFile` (preview and save, with `Range`), `knowledge.createWorkspaceFileStream` (a player's ticket, DECISIONS §98), `knowledge.downloadWorkspaceFolder` (zip), `sessions.list` (Attach to chat targets) | `knowledge.uploadWorkspaceFile`, `knowledge.createWorkspaceFolder`, `knowledge.writeWorkspaceText` (edit with etag, new file with `null`), `knowledge.moveWorkspaceFile` (rename, move), `knowledge.copyWorkspaceFile`, `knowledge.deleteWorkspaceFile`, `knowledge.attachWorkspaceFile` (then the composer) | — |

## Root content with a payload

| # | Destination | Reads | Acts | Events |
|---|---|---|---|---|
| 17 | `conversation` — Conversation (transcript, composer, session settings sheet, interaction cards, location consent, media players, message actions) | `sessions.get` (session + active/queued runs + pending approvals + context), `sessions.listMessages` (backwards paging), `sessions.listRuns`, `sessions.getRun`, `sessions.getTrajectory` (the Trajectory tab and its session log), `sessions.listFiles` and `sessions.readFile` (the Files list and the preview panel beside the chat), `sessions.createFileStream`, `sessions.createAttachmentStream` and their tickets (`knowledge.streamFile`, `sessions.streamAttachment`) for a video or a sound, played with ranges (DECISIONS §90, §98), `sessions.listChanges`, `sessions.getRunChanges` and `sessions.getRunChangeDiff` (the "files changed" card under a run's reply and its diff in the panel; while the run goes, the same card "so far", DECISIONS §102), `sessions.getContextBreakdown` (the context meter's window by category, §102), `sessions.listSubagents` + `sessions.tailSubagent` (the Subagents panel, DECISIONS §56), `sessions.getApproval`, `models.listCatalogue`, `devices.listRequests` (`status=pending`), `devices.getRequest` | `sessions.createRun` (`when: queue / next / interrupt`), `sessions.cancelRun`, `sessions.update` (model, reasoning effort, working dir, notify, title, category, archive), `sessions.fork`, `sessions.export`, `sessions.delete`, `sessions.respondApproval`, `sessions.interruptSubagent`, `sessions.steerSubagent`, `devices.respondRequest` (share / decline location), attachments and voice rows above | `message.created`, `message.delta`, `reasoning.delta`, `tool.started`, `tool.completed`, `tool.failed`, `run.queued`, `run.started`, `run.completed`, `run.failed`, `run.cancelled`, `approval.requested`, `approval.resolved`, `context.updated`, `subagent.started`, `subagent.updated`, `subagent.completed`, `session.updated`, `session.deleted`, `request.created`, `request.completed` |
...
| 26 | `channels` — Channels (per-platform fields, enable/disable, clear credentials, QR login; the «ويب هوك» section, DECISIONS §97) | `agents.listChannels` (fields served per platform), `agents.listWebhooks` | `agents.updateChannel`, `agents.clearChannel`, `agents.loginChannel`, `agents.restart`, `agents.createWebhook`, `agents.deleteWebhook`, `agents.testWebhook` | `agent.updated`, `job.queued`, `job.progress` (QR), `job.completed`, `job.failed` |
...
| 29 | `presets` — Presets (a card on the agent's Settings page, DECISIONS §100) | `agents.listPresets`, `agents.getPreset` | `agents.createPreset`, `agents.activatePreset`, `agents.deletePreset` | `agent.updated`, `job.queued` (a restart the settings ask for) |
...
| 33 | `hermesSettings` — Settings (Hermes's own keys per profile: Agent runtime / Memory / Approvals / Network proxy / Privacy from `sections`, each field with `help` and Hermes's `default`; §58) and «Waiting for review» | `agents.getSettings`, `agents.listPendingWrites` | `agents.updateSettings` (one section per save; `restart_job_id` for the default profile's proxy), `agents.approvePendingWrite`, `agents.rejectPendingWrite` | `agent.updated`, `job.*` |
