...
| 5 | `models` — Models › **General Models** | `models.listProviders`, `models.getDefaults` | `models.createProvider`, `models.updateProvider` (key, base URL, visibility), `models.deleteProvider`, `models.refreshProvider`, `models.testProvider`, `models.putModel` (alias, visibility, context, custom), `models.deleteModel`, `models.startProviderSignIn`, `models.getProviderSignIn`, `models.completeProviderSignIn`, `models.setDefaults` (default + fallbacks) | `job.completed` (refresh) |
...
| 10 | `settings` › **Webhooks** (admin) | `notify.listWebhooks`, `notify.listWebhookEvents` | `notify.createWebhook`, `notify.updateWebhook`, `notify.deleteWebhook`, `notify.testWebhook` | `job.completed` (test) |
...
| 25 | `kanban` — Kanban (board picker, stats chips, columns, card, drop sheets, task detail with subtasks/comments/runs/activity) | `board.listProjects`, `board.getProject`, `board.get` (columns + counts in one call), `board.listTasks` (filters), `board.getTask`, `board.listActivity`, `board.getWorktree`, `agents.list` (assignees) | `board.createProject`, `board.updateProject`, `board.deleteProject`, `board.createTask`, `board.updateTask`, `board.moveTask` (every drop; `409 state_invalid` lists allowed targets), `board.assignTask`, `board.unassignTask`, `board.stopTask`, `board.bulkUpdateTasks` (archive), `board.bulkDeleteTasks`, `board.deleteTask`, `board.createSubtask`, `board.updateSubtask`, `board.deleteSubtask`, `board.setDependencies`, `board.createComment`, `board.createWorktree`, `board.deleteWorktree`, `board.dispatch` | `project.created`, `project.updated`, `project.deleted`, `task.created`, `task.updated`, `task.moved`, `task.deleted`, `task.assigned`, `task.unassigned`, `subtask.updated`, `comment.created`, `worktree.updated`, plus `run.*` on the task session |
