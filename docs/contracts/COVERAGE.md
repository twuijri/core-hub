...
| 12 | `usage` — Usage | `audit.getUsage` (`days`, `profiles=all`, `agent_id`, `utc_offset_minutes`; decision §47); `preferences.show_cost` for the cost toggle | — | — |
| 13 | `performance` — Performance (admin) | `audit.getReport` (`kind=performance`) — Phase 4 stub, pollable | — | — |
| 14 | `skillsUsage` — Skills Usage | `audit.getSkillUsage` (`days`, `profiles=all`, `agent_id`, `utc_offset_minutes`; decision §47); `getReport` `kind=skills` answers the same body | — | — |
