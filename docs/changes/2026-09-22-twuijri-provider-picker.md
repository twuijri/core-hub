...
- **فصل «ما أضفته» عن «ما يمكن إضافته»** (قرار العقد §27): صف في `providers` يوجد لأن أحدًا
...
مسجَّل في `docs/contracts/DECISIONS.md` §27. كل عملية قائمة بقيت تعمل كما هي.
...
**الوثائق** — `docs/contracts/DECISIONS.md` §27، `docs/contracts/COVERAGE.md`،
...
## لقطات
شاشة النماذج والحوار جُرِّبا فعليًا على المركز الحقيقي (هارنس `packages/web/e2e/hub.ts`
بمتصفّح حقيقي، بالعربية RTL): الشاشة الفارغة بزرّها، والحوار وقد عبّأ عنوان LM Studio،
ونصّ الخطأ الحقيقي من زر «اجلب» على عنوان لا أحد يستمع عليه:
`لم يُرجِع العنوان قائمة نماذج. — fetch failed`، ثم البطاقة بعد الإضافة وفيها الخطأ نفسه
لا خطأً مخترعًا عن مفتاح ناقص. سيناريو اللقطات مؤقّت ولم يُلتزم (رحلات Playwright الثلاث
الحتمية هي وحدها الملتزمة).

## التسليم والخطوة التالية
- ستة التزامات على `feat/provider-picker` من `origin/main` (عقد · خادم · CLI · ويب · وثائق ·
  تعديل مظهري). **لم يُدفع ولم يُفتح PR.**
