...
## القرار والموافقات
قرار العقد §57 (مقترح — ينتظر تأكيد المالك):
...
- الخادم: `packages/server/src/modules/rooms/{schema,store,serialize,service,realtime,index}.ts`
  و`drizzle/0017_rooms.sql` (أعمدة جديدة لـ`rooms` و`seats` و`room_messages`، وجدولا
  `room_handoff_chains` و`seat_presets`؛ رقم 0017 لأن 0016 محجوز في عدة طلبات مفتوحة).
...
- الويب: `packages/web/src/rooms/*`، ومسار `/rooms/:roomId?` و`/join/:code`، وقائمة الغرف في
  ...
  والإنجليزية.
- الوثائق: DECISIONS §57، `docs/domain/rooms.md`، `docs/STATUS.md` (226 من 264).
