...
- أُعيد تركيب الفرع فوق `feat/auth-and-agents` (طلب الدمج #3) بلا تعارضات. اختبار «الوكلاء 501» لم يعد صحيحاً بعد وصول موديول الوكلاء، فاستُبدل باختبار يتحقق أن الكتالوج المعتمد يعرض هرمس (بالاسم المختصر `hermes` ومعرّف ULID) وأن وكيلاً غير موجود يُرفض بـ`not_found`.
- الفحوص بعد التركيب: lint وtypecheck وbuild نظيفة، الوحدات 11 + 55 + 153، العقد 247.
