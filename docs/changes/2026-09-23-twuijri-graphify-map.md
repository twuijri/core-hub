...
- **`pnpm graph` (`scripts/graph.mjs`) بدل `graphify update .`** لثلاثة أسباب وُجدت بالتجربة:
  ...
  ٠. Graphify يربط الاستيرادات بما يجده على القرص، فنسخة فيها `node_modules` وحزم مبنية (`dist/`،
     عميل العقد المولَّد) تخرج بعقد لا تخرج بها نسخة نظيفة — أسقط فحص CI أول مرّة بـ ٢٦ عقدة
     (٦٢٥٩ عندي، ٦٢٣٦ في CI؛ أعدت إنتاج رقم CI بنسخة نظيفة). فالسكربت يبني في نسخة مؤقتة من
     ملفات git المتتبَّعة والجديدة غير المستثناة فقط.
