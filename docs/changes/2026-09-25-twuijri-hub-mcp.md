...
## القرار والموافقات
كل ما يلي **مقترح — ينتظر تأكيد المالك** (قرار العقد §67؛ المالك نائم، فاتُّخذ القرار وسُجّل):
...
- قرار العقد §67 في `docs/contracts/DECISIONS.md`.
...
- الخادم: `packages/server/src/modules/agents/hub-tools/` (`catalog.ts` الأدوات الـ18،
  ...
  `drizzle/0019_hub_tools.sql` (جدولا `hub_tool_settings` و`hub_tool_calls`)؛ منفذ التنبيه في
  `modules/index.ts`.
...
- الوثائق: `DECISIONS.md` §67، `STATUS.md` (209 من 267)، `docs/domain/agents.md`.
...
## المخاطر والرجوع
- **الترحيل `0019`** قد يتصادم رقمه مع فروع مفتوحة أخرى تضيف ترحيلًا؛ عند الدمج يُعاد توليده
