...
جُرّبت الواجهة يدويًا على مركز حقيقي (تسجيل دخول، اختيار وكيل، إرسال رسالة، سمة داكنة، عربية) فظهر أمران وأُصلحا:
- عند فشل التشغيل كانت المنصة تُبقي رسالة مساعد فارغة بحالة `failed`، فيختفي إشعار السبب ولا يبقى إلا وسم «فشل» بلا تفسير. صار الإشعار يُخفى فقط إذا كانت الرسالة الفاشلة تحمل نصًا فعليًا.
- شرائح قدرات الوكيل كانت تُعرض بأسماء إنجليزية خام (منها `kanban`)؛ صارت مترجمة في اللغتين مع اسم «المهام»، مع الرجوع إلى الاسم الخام لأي قدرة جديدة لا ترجمة لها.
الفحوص بعد الإصلاح: lint وtypecheck وbuild نظيفة، الوحدات 11 + 89 + 55 + 193 + 73، i18n سليم. الصورة مع الويب: 764 ميغابايت غير مضغوطة، نحو 208 ميغابايت عند السحب.

## تحديث لاحق ٢ (المنسّق، 2026-09-21): رحلات Playwright كانت تعتمد على الجهاز
سقطت الرحلات الثلاث في CI ونجحت محليًا. السبب: مركز الاختبار كان يفحص بوابة هرمس الحقيقية،
فإن وُجدت على جهاز المطوّر ظهر هرمس «متاح» ونجحت، وعلى عدّاء CI (بلا هرمس) صارت كل بطاقات
منتقي الوكلاء معطّلة فانتهت النقرة بانتهاء المهلة. أُصلح بجعل `e2e/hub.ts` مستقلًا عن الجهاز:
مثبِّت يقول إن وكلاء الكتالوج موجودون، وبوابة هرمس مكتوبة بالسيناريو تردّ `ok` دائمًا — والأدوار
نفسها تبقى من `ScriptedRunner`، لا من وكيل حقيقي. أُعيد إنتاج الفشل محليًا (ببوابة ترفض) قبل الإصلاح.
وأُضيف في شاشة «محادثة جديدة» إشعار يشرح السبب حين تكون كل البطاقات معطّلة، مع رابط إلى مدير الوكلاء.
الفحوص بعد الإصلاح: lint وbuild نظيفة، الوحدات 73 للويب، ورحلات Playwright الثلاث تنجح.
