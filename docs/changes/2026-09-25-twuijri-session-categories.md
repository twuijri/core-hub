...
CI على GitHub (طلب الدمج #121):
- الدفعة الأولى: فشلت رحلة Playwright الجديدة وحدها في الجولة الكاملة — المحادثة الثانية (لخطوة
  السحب) كانت تُنشأ بالمُلحِّن مرة ثانية، وزر الإرسال صار معطّلًا لحظة الضغط (سباق مع إعادة ضبط
  المسودة عند الانتقال من محادثة إلى «محادثة جديدة»). الإصلاح: المحادثة الثانية تُنشأ عبر الـAPI.
- بعد الإصلاح (الالتزام `72ac71e`):

```
Docker image builds and answers /health — pass (2m42s)
Lint, typecheck, contracts, tests, build — pass (15m43s)
PR adds or updates a change record — pass (16s)
PR leaves graphify-out/ to the code-map bot — pass (10s)
Web smoke journeys (Playwright against the real hub) — pass (5m14s)
...
## المخاطر والرجوع
- رقم الترحيل كان `0016`، وأخذه أيضًا #108 و#110؛ في دمج التكامل (2026-09-25) صار
  `0018_session_categories` بعد `0016_run_file_changes` و`0017_usage_analytics`. الجدول جديد ولا يمس
  جداول أخرى.
