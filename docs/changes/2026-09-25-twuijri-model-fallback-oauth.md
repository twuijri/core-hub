...
## القرار والموافقات
**مقترح — بانتظار تأكيد المالك** (DECISIONS §49 و§50):
...
- `docs/contracts/DECISIONS.md` §49 و§50.
...
- **ملكية `fallback_providers`**: حيث للبروفايل نموذج محادثة في المركز، يكتب المركز السلسلة —
  سلسلة كتبها أحد بـ `hermes fallback` تُستبدل (تُفرَّغ إن كانت سلسلة المركز فارغة). مذكور في §49.
...
- المالك: تأكيد §49 و§50 (خصوصًا: عدم الانتقال عند 401/403، ملكية `fallback_providers`، ترك
