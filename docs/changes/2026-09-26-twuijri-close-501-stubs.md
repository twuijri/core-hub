...
المسؤول: twuijri · الفرع: feat/close-501-stubs · الحالة: review
...
CI على #144 بعد دمج المجموعات الثلاث (الرأس `a21b1fc`، وفيه أيضًا عمل تنظيف رموز الدفع المدموج قبله): ١٧ فحصًا كلها ناجحة
(الخادم بشرائحه الثلاث، العقد والعملاء، رحلات الويب، الترحيل على SQLite وPostgreSQL، الصورة، سطح المكتب، أندرويد وiOS).
تشغيل المجموعة ٢ وحدها (`e91cf33`) أُلغي لأن دفعة أحدث حلّت محلّه، فلا نتيجة مستقلة له.
...
بُنيت ثلاث مجموعات من (أ) ودُمجت في `night/2026-09-26` (#144): رحلة الوكيل، طلبات قدرات الجهاز، صورة الوكيل — ٦ عمليات
من ٢٠ (الآن ٣٠١ من ٣١٥). توقفت هنا ليبقى الطلب قابلًا للمراجعة.

ما بقي:
- **من (أ):** مجموعة `usage` في أدوات MCP، ومجموعة `devices` فيها (تُربط الآن بـ`devices.createRequest`)، وانتظار المهمة
  لاعتماداتها قبل `auto_start`، والصوت في تطبيق سطح المكتب.
- **أسئلة (ب) للمالك:** ملفات إعداد الوكيل (من يحرّرها وفي أي بيت)، المرحّل (هل تريده، وبأي مسار)، مجموعة `browser`،
  من يملك تشغيل رسالة القناة لأدوات المركز، ntfy، محوّل «الحزام».
- **مرشّحات (ج) للحذف:** Presets (٤ عمليات)، Peers (٥ عمليات)، وجملة إنشاء بوت تيليجرام في STATUS.
- قرارات للتأكيد: DECISIONS §73 و§74 و§76.
