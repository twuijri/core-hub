...
- **إعادة التسمية إلى Majlis** (`450db21`): أُعيد بناء الفرع فوق `origin/main` الجديد
  بلا تعارضات؛ كل ما أضافه هذا الفرع يستعمل الأسماء الجديدة (`@majlis/*`، «Majlis»
  اسمًا لمؤلّف رسائل النظام، `majlis.workspace` / `majlis.user` نطاقَي اشتقاق
  المعرّفات المؤقّتة). ما بقي من الاسم القديم موجود فقط في ملفات مولَّدة تُعاد كتابتها
  عند كل بناء.
- **ADR 0007 (العملاء من الصفر على العقد)**: لم يُفتَح أي ملف من تطبيقَي الهاتف
  القديمين أثناء هذه المهمة، ولم يُشكَّل أي مسار أو حدث أو سلوك استئناف على مقاسهما.
  الاستئناف مشتقّ من رقم `seq` في ظرف الحدث كما يعرّفه `events/README.md`، وتسلسل
  الأحداث هو بالضبط ما تعلنه `x-rt-events` لعملية `sessions.createRun` ومخطّطات
  `events/sessions/*` — وهو ما يحتاجه عميل `packages/cli` المرجعي المولَّد من العقد.
...
3. بعدها: تشغيل حقيقي واحد مع Hermes للتحقّق من ترتيب الإطارات وأرقام الاستهلاك،
   ثم إثبات المرحلة صفر بعميل `packages/cli` المرجعي المولَّد من العقد (ADR 0007،
   يُكلَّف به فرع مستقل) — لا بتطبيقات الهاتف القديمة.
4. مؤجَّل بوعي: `session_categories`، المرفقات (`knowledge`)، نواة الوظائف
   و`/rt/jobs`، وتقارير `audit`.

## تحديث لاحق (المنسّق، 2026-09-21)
- أُعيد تركيب الفرع فوق `feat/auth` (طلب الدمج #2) وحُلّت التعارضات: قائمة `ErrorCode` تُؤخذ من العقد حرفياً، وملفا الترجمة يحملان كتلتي `auth` و`sessions` معاً، و`pairing.ts` يستخدم الكود `internal`.
- بما أنه لا توجد نسخة منشورة بعد، جُمع المخطط في ترحيل واحد `0000_tired_martin_li.sql` بدل ترحيلين متصادمين؛ `db:generate` بعدها يقول «لا تغييرات».
- الفحوص بعد التركيب: lint وtypecheck وbuild نظيفة، الوحدات 11 + 107، العقد 247، i18n وcontracts:lint سليمة.
