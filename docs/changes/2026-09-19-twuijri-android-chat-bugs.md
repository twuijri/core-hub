...
## القرار والموافقات
- الموافقة: تكليف المالك بإصلاح البلاغات الثلاثة على فرع مهمة من `origin/mobile`،
  بلا دفع وبلا PR وبلا أي مساس بـ `clients/ios`.
- القرار: مطابقة الويب حرفيًّا في تعيين نص الرسالة (`display_content ?? content`
  بلا تسطيح) بدل ترقيع المسطِّح، لأن المسطِّح هو مصدر ثلاثة أعراض مختلفة.
  البديل المرفوض: إبقاء التسطيح مع استثناءات لكل شكل كتلة، وهو ما يعيد إنتاج
  العيب مع أول شكل جديد.
- القرار: الجلسة المفقودة حالةٌ لا فشل تشغيل — تُمسح ويُعرض فراغ محايد، بدل
  صفّ أحمر. هذا يغيّر سلوكًا ظاهرًا للمستخدم ويحتاج نظر المالك.
- ينتظر إذن المالك: الدفع، فتح PR، الدمج، وبناء أي صورة.
...
## المخاطر والرجوع
- سلوكي: صفوف كانت تُسقَط صارت تظهر، ونصّ «📎 اسم» صار بطاقة تنزيل؛ هذا هو
  المقصود، لكنه يغيّر شكل محادثات قديمة.
- سلوكي: تصحيح اتجاه الترقيم يغيّر أي صفحة تُفتح أوّلًا في محادثة طويلة.
  `loadOlderHistory` لا يستدعيه أي عنصر واجهة اليوم، فالخطر محصور في
  `loadLatestPage`.
- بيانات: `forgetMissingSession` يمسح المعرّف المخزَّن للملف الشخصي فقط؛ لا
  يحذف شيئًا على الخادم.
- الرجوع: إعادة الفرع إلى `origin/mobile` (`6d6afe60`)، أو إرجاع الالتزام
  الثاني وحده لاستبقاء إصلاح الدرج.
