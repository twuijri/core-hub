...
## تحديث لاحق 2 (المنسّق، 2026-09-21): مراجعة أرقام الكتالوج
- تُحقِّق من الإصدارات المثبَّتة على سجل npm: القديمة كانت موجودة لكنها متأخرة سنة عن الحالي. حُدّثت إلى: `@zed-industries/claude-code-acp@0.16.2`، `@google/gemini-cli@0.60.0`، `opencode-ai@1.18.31`.
- Codex: حزمة `@openai/codex` لا تعرض نقل ACP بنفسها، فاستُبدلت بمحوّل Zed الرسمي `@zed-industries/codex-acp@0.16.0` (الثنائي `codex-acp` يتكلم ACP على stdio مباشرة بلا وسيطات).
- ما زال غير مُجرَّب على جهاز حقيقي: أن كل ثنائي يبدأ فعلاً بوسيطات ACP المذكورة (`gemini --experimental-acp`، `opencode acp`). الفحوص بعد التعديل: الوحدات 153، lint وtypecheck نظيفة.
