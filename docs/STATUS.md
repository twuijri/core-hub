...
- **Android** (`apps/android`, Kotlin + Compose, since 2026-09-25, part 1 of 3): pairing by the
  ...
  app runs, and a background check every 15 minutes while it is closed (no push: `devices` is still
  501, so FCM is a follow-up); «Share to Core Hub» turns shared text into a new chat's draft (files
