...
- **Web** (`packages/web`): first-run setup, login, chat with streaming,
  ...
  says so in the history; since 2026-09-25 a «جداول شائعة» / "Common schedules" menu fills in
  ...
  limits it ran under, what it cost and which limit stopped it, and edits the workflow's limits
  — the workflow editor of #109 is not merged, so that form lives in the run's view for now), notifications, people, workspaces (with export to a download and import
