...
**231 of 264 contract operations are implemented.** Nothing fakes a success:
...
`rooms` answers all but its memory operations (part 3), and the Hermes-gateway half of `agents` is still 501. Phase 3 (phones and desktop) is last, by the
...
- **Web** (`packages/web`): first-run setup, login, chat with streaming,
  ...
  the manager has Room settings (`@all`, handoffs and their limit) and Clear the context.
