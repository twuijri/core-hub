...
**216 of 274 contract operations are implemented.** Nothing fakes a success:
...
time came or that someone ran now — the hub's own scheduler for every schedule but
...
- **Web** (`packages/web`): first-run setup, login, chat with streaming,
  ...
  sessions list, agents, models, the Tasks board,
  ...
  reaches whoever runs it), an approval's timeout, attachments on a step), notifications, people, workspaces (with export to a download and import
