...
|---|---|---|
| جدول توجيه دور × نوع مهمة → وكيل، مع تجاوز بحسب المرحلة | `board` (assignment rules) + `agents` (roles/capabilities) | قواعد لكل مشروع تُخزَّن وتُحرَّر من الواجهة؛ الوكيل يعلن أدواره في السجل |
| «لا تشغيل بلا مسار مشروع» (fail-closed) | `board` | لا run بلا worktree/مستودع محلول؛ الخطأ `{ error, code }` |
| مرحلة مراجعة تلقائية بوكيل مختلف بعد اكتمال التنفيذ | `board` + `rooms` | انتقال `in_review` يستدعي مقعد المراجع (كما في clawboard) |
| إدخال بطاقة من رسالة دردشة (`#` بادئة) عبر قناة | `board` + محوّل Hermes (قنوات) | أمر في قناة Hermes ينشئ مهمة عبر العقد |
...
| سجل أحداث لكل بطاقة | `board` (timeline) + `audit` | حدث `task.moved`, `run.*` مخزَّن |
