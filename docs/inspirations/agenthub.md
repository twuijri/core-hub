...
| مفردات صارمة: `task` (ملكية) / `attempt` (محاولة) / `run` (تقسيم تنفيذ) | `tasks` + العقد | نستخدم `task` و`run` ونوثّق الفرق في العقد |
...
| تنشيط حلقي: العملية قد تخرج والمهمة تستمر من حالة دائمة | `schedules` + `tasks` | run لا يفترض عملية حيّة؛ إعادة التنشيط حدث |
| Kanban هو سطح مهام الفريق، والقنوات/الخيوط للتنسيق | `rooms` + `tasks` | الغرفة تشير إلى مهام قسم «المهام» ولا تنسخها |
