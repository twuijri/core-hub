...
alias corehub='node /path/to/corehub/packages/cli/dist/bin.js'
```
...
|---|---|---|
| `corehub setup --server URL [--username NAME] [--display-name N] [--workspace-name N]` | First run only (ADR 0011): creates the owner account on a hub that has none. Asks for the hub's setup token (printed in its log, and in `<DATA_DIR>/setup-token.txt`) and for the password, twice and hidden. Ends signed in. | `auth.getSetup`, `auth.completeSetup` |
| `corehub login --server URL [--username NAME]` | Signs in; the password is always prompted (hidden on a terminal, read from stdin otherwise). Stores the tokens. | `meta.get`, `auth.login` |
| `corehub logout` | Revokes the token on the hub and forgets it locally. | `auth.logout` |
| `corehub whoami` | The signed-in user, hub, workspace and token kind. | `auth.getMe` |
| `corehub pair [--ttl S] [--connection lan\|relay] [--no-qr]` | Creates a pairing, prints the code, the QR and the exact `pair claim` line, then waits for `pairing.claimed` on `/rt/devices` (polling `auth.getPairing` as a fallback). | `auth.createPairing`, `auth.getPairing` |
| `corehub pair claim '<QR JSON>'` or `corehub pair claim CODE --pairing-id ID --server URL` | The device side: claims the pairing as this computer and stores the app token. | `auth.claimPairing` |
| `corehub agents list [--kind K]` / `get ID` | The registry. | `agents.list`, `agents.get` |
| `corehub agents install ID` / `remove ID` | Start the install/uninstall job. | `agents.install`, `agents.uninstall` |
| `corehub sessions list [--limit N] [--cursor C] [--archived true\|false\|all] [--agent ID] [--q TEXT]` | Sessions of the workspace, paged. | `sessions.list` |
| `corehub sessions new --agent ID [--title T] [--working-dir D] [--model M]` | Creates a session and prints the `chat` command for it. | `sessions.create` |
| `corehub sessions show ID` / `delete ID` | The live session document / delete. | `sessions.get`, `sessions.delete` |
| `corehub chat ID [--message TEXT] [--once] [--reasoning] [--approve D] [--timeout S]` | The streamed conversation (below). | `sessions.get`, `sessions.createRun`, `sessions.cancelRun`, `sessions.respondApproval`, `sessions.getRun` |
...
on stdout; prompts and notices go to stderr, so `corehub sessions list --json | jq` works.
...
`$XDG_CONFIG_HOME/corehub/config.json`, or `~/.config/corehub/config.json` (override with
`--config PATH` or `COREHUB_CONFIG`). The directory is created `0700`, the file is written `0600`
...
```
$ corehub chat 01J8QK3ZR2W7M5N4P6T8V9X0YA
...
- `COREHUB_DEBUG=1` logs the socket lifecycle (connect, subscribe acks, drops) on stderr.
...
Run `pnpm --filter @corehub/cli test`; `docs/harness/validation.md` lists the rest.
