...
private struct AccountView: View {
    ...
    var body: some View {
        List {
            ...
            Section("Avatar") { PhotosPicker(selection: $photo, matching: .images) { SettingsRow(icon: "photo.fill", color: .purple, title: "Upload image") }.onChange(of: photo) { _, item in Task { await upload(item) } }; Button { Task { await store.attempt({ try await store.api.resetAvatar() }); store.currentUser = await store.attempt({ try await store.api.currentUser() }) } } label: { SettingsRow(icon: "arrow.counterclockwise", color: .gray, title: "Reset avatar") } }
...
private struct ProfileDetailView: View {
    ...
    var body: some View { List {
        ...
        Section("Runtime status") { LabeledContent("Bridge") { StatusPill(text: runtime?.bridgeRunning == true ? String(localized: "Running") : String(localized: "Stopped"), color: runtime?.bridgeRunning == true ? .green : .gray) }; LabeledContent("Gateway") { StatusPill(text: runtime?.gatewayRunning == true ? String(localized: "Running") : String(localized: "Stopped"), color: runtime?.gatewayRunning == true ? .green : .gray) }; if let url = runtime?.gatewayURL.nilIfEmpty { Text(url).font(.caption.monospaced()).textSelection(.enabled) }; Button("Restart runtime") { Task { await store.attempt({ try await store.api.restartProfileRuntime(profile.name) }); await load() } }; Button("Restart gateway") { Task { await store.attempt({ try await store.api.restartGateway(profile: profile.name) }); await load() } } }
        Section { Button(profile.name == store.selectedProfile ? "Active profile" : "Make active profile") { Task { do { try await store.api.activateProfile(profile.name); store.chooseProfile(profile.name); await store.refreshProfiles() } catch { store.errorMessage = error.localizedDescription } } }.disabled(profile.name == store.selectedProfile) }
        Section("Avatar") { PhotosPicker(selection: $photo, matching: .images) { Label("Upload profile avatar", systemImage: "photo") }.onChange(of: photo) { _, item in Task { await upload(item) } }; Button("Reset profile avatar", role: .destructive) { Task { await store.attempt({ try await store.api.resetProfileAvatar(profile.name) }); await store.refreshProfiles() } } }
