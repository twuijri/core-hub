    ...
    var body: some View {
        List {
            Section("App Relay") {
                ...
                Picker("Route", selection: Binding(get: { relay?.route ?? "official" }, set: { route in Task { relay = await store.attempt({ try await store.api.appRelay("route", method: "PUT", body: ["route": route]) }) } })) { Text("Official").tag("official"); Text("Cloudflare").tag("cloudflare") }
            ...
            Section("App connections") {
                Button("Create LAN pairing code") { Task { let value = await store.attempt({ try await store.api.appAuthorization(cloud: false) }); authorization = value?.string("authorization_code", "qr_payload") ?? "" } }
                Button("Create cloud matching code") { Task { let value = await store.attempt({ try await store.api.appAuthorization(cloud: true) }); authorization = value?.string("matching_code", "qr_payload") ?? "" } }
            ...
            Section("Studio devices") {
                ...
                ForEach(devices) { device in VStack(alignment: .leading, spacing: 6) { HStack { Text(device.name).font(.headline); Spacer(); StatusPill(text: device.online ? String(localized: "Online") : String(localized: "Offline"), color: device.online ? .green : .gray) }; Text(device.url).font(.caption.monospaced()).foregroundStyle(.secondary); HStack { if device.inbound == "pending" { Button("Approve") { Task { await store.attempt({ try await store.api.deviceAction(device.id, action: "approve") }); await load() } }; Button("Reject") { Task { await store.attempt({ try await store.api.deviceAction(device.id, action: "reject") }); await load() } } }; if device.inbound == "blocked" { Button("Unblock") { Task { await store.attempt({ try await store.api.deviceAction(device.id, action: "unblock") }); await load() } } } else { Button("Block") { Task { await store.attempt({ try await store.api.deviceAction(device.id, action: "block") }); await load() } } }; if device.inbound == "approved" || device.outbound == "approved" { Button("Connect") { Task { await store.attempt({ try await store.api.deviceAction(device.id, action: "connect") }); await load() } } } } } }
            }
            Section("Peer connections") {
                ForEach(peers) { peer in HStack { VStack(alignment: .leading) { Text(peer.name); Text(peer.url).font(.caption.monospaced()).foregroundStyle(.secondary) }; Spacer(); Button("Disconnect", role: .destructive) { Task { await store.attempt({ try await store.api.disconnectPeer(peer.id) }); await load() } } } }
            }
        }.navigationTitle("Connections").overlay { if loading { ProgressView() } }.task { await load() }.refreshable { await load() }.alert("Request device by URL", isPresented: $requesting) { TextField("https://device.local", text: $manualURL); Button("Request") { Task { await store.attempt({ try await store.api.requestDevice(url: manualURL) }); await load() } }; Button("Cancel", role: .cancel) {} }
    }
    private func load() async {
        loading = true
        // Sequential so every failure is reported through the shared banner.
        relay = await store.attempt({ try await store.api.appRelay() }) ?? relay
