...
struct KanbanView: View {
    ...
    private func loadOperations() async { stats = (await store.attempt({ try await store.api.kanbanStats(board: selectedBoard) })) ?? [:]; diagnostics = (await store.attempt({ try await store.api.kanbanDiagnostics(board: selectedBoard) })) ?? [] }
